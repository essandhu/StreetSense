...
async def test_unknown_segment_returns_404(api_client: AsyncClient) -> None:
    bogus = uuid4()
    resp = await api_client.get(f"/api/cities/cambridge/segments/{bogus}")
...
async def test_segment_detail_is_deterministic_across_calls(
    ...
    a = (await api_client.get(f"/api/cities/cambridge/segments/{seed_segment}")).json()
    b = (await api_client.get(f"/api/cities/cambridge/segments/{seed_segment}")).json()
